...
export const XButton = ({ label, icon, onClick, loading }) => {
  return (
    <div className="w-full">
      <button
        className="flex items-center justify-center bg-blue-600 text-white rounded font-semibold w-full py-2"
        onClick={onClick}
        disabled={loading}
      >
        {loading ? (
          <div className="flex items-center justify-center w-6 h-6">
            <div
              className="spinner-border animate-spin inline-block w-6 h-6 border-4 rounded-full text-white"
              role="status"
            >
...
const Signup = () => {
  ...
  const signUp = async (e) => {
    ...
    await axios
      .post(/*`${path}`*/ "http://localhost:3000/auth/register", userBody, {
    ...
    <div>
      ...
      <div className="flex flex-col items-center min-h-screen bg-gray-100">
        <div className="flex flex-col items-center w-full max-w-md bg-white p-6 rounded-lg shadow-md mt-6">
          <h1 className="text-blue-600 text-3xl font-bold mb-4 ">Sign Up</h1>
          ...
          <XTextfield
            ...
            icon={<AiOutlineLock />}
            suffixIcon={
              toggleIconState ? <AiOutlineEye /> : <AiOutlineEyeInvisible />
            }
            validation={validatePasswordLength}
            onValueChange={handlePasswordLength}
            onClick={toggleIcon}
            inputType={toggleIconState ? "text" : "password"}
          />
          <Spacer height={5} />
          {passwordState && (
            <p
          ...
          <XTextfield
            ...
            inputType={toggleIconConfirmState ? "text" : "password"}
          ...
          {confirmPasswordState && (
            <p
              className={`text-${
                passwordMatch ? "bg-green-400" : "bg-red-700"
              }-500 font-semibold mt-2`}
            >
          ...
          <div className="flex items-center justify-center">
            ...
            <Spacer width={5} />
            <Link to="/auth/Login">
              <h2 className="text-blue-600  font-semibold">Sign In</h2>
            </Link>
          </div>
        </div>
      </div>
